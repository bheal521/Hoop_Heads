...
## create node / links dataframes
usernames <- unique(c(quote.summary2$poster, quote.summary2$Quote))
id <- c(1:length(usernames))
MisNodes1 <- data.frame(usernames = usernames, id = id, group = 1, size = 1)

quote.summary2 <- merge(x = quote.summary2, y = MisNodes1[, c("usernames", "id")], by.x = "poster", by.y = "usernames")
names(quote.summary2) <- c("poster", "Quote", "Sum", "source")
quote.summary2 <- merge(x = quote.summary2, y = MisNodes1[, c("usernames", "id")], by.x = "Quote", by.y = "usernames")
names(quote.summary2) <- c("poster", "Quote", "Sum", "source", "target")

quote.summary2 <- quote.summary2 %>%
  arrange(source)

forceNetwork(Links = quote.summary2, Nodes = MisNodes1, Source = "poster", Target = "Quote", 
             zoom = TRUE, Value = "Sum", NodeID = "usernames", Group = "group")
```
